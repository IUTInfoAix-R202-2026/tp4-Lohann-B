...
public class PokemonController {
  ...
  private void initialize() {
    ...
    // Affichage du TableView
    colNumero.setCellValueFactory(
        c -> new SimpleStringProperty(String.valueOf(c.getValue().numero())));
    colNom.setCellValueFactory(c -> new SimpleStringProperty(String.valueOf(c.getValue().nom())));
    colType.setCellValueFactory(c -> new SimpleStringProperty(String.valueOf(c.getValue().type())));

    table.setItems(viewModel.pokemonsProperty());

    labelResume.textProperty().bind(viewModel.resumeProperty());

    Bindings.bindBidirectional(viewModel.rechercheProperty(), champRecherche.textProperty());
